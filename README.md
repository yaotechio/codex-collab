...
一步装齐：`codex` MCP server、`/codex-collab` 工作流命令、写操作确认 hook。Server 是预编译好的 Go 二进制、经 npm 分发——插件用 `npx` 启动它，首次运行自动下载对应平台的二进制并缓存。**无需 Go 工具链。**

**前置依赖**：[Node.js](https://nodejs.org/) ≥ 16（提供 `npx`）与 PATH 中的 [Codex CLI](https://github.com/openai/codex)（已 `codex login`）。

发布新版本后，执行 `/plugin update codex-collab@yaotechio` 升级（Claude Code 启动时也会自动更新插件）。要改默认值，在启动 Claude Code 前导出对应环境变量即可（插件读取 `${CODEX_MCP_MAX_ROUNDS:-6}` 等，详见[配置](#配置)）。
