...
一步装齐：`codex` MCP server、`/codex-collab` 工作流命令、写操作确认 hook。Server 通过 `go run` 直接从源码运行（钉在发布版本上；首次调用编译并缓存，约 5–15 秒，之后秒起），无需手动编译或配置。

**前置依赖**：[Go](https://go.dev/) ≥ 1.23 与 PATH 中的 [Codex CLI](https://github.com/openai/codex)（已 `codex login`）。

发布新版本后，执行 `/plugin update codex-collab@codexmcp` 升级（Claude Code 启动时也会自动更新插件）。要改默认值，在启动 Claude Code 前导出对应环境变量即可（插件读取 `${CODEX_MCP_MAX_ROUNDS:-6}` 等，详见[配置](#配置)）。
