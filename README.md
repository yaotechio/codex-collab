...
一步装齐 `codex` MCP server、`/codex-collab:flow` 命令与写操作确认 hook（需 [Node.js](https://nodejs.org/) ≥ 16；首次运行需几秒初始化）。
...
> 命令完整名为 `/codex-collab:flow`（插件名 `codex-collab` + 命令名 `flow`），在 `/plugin` 选择器里即如此显示。
...
**协作命令**：把 `.claude/commands/flow.md` 复制到 `~/.claude/commands/`（Windows：`%USERPROFILE%\.claude\commands\`），即可用 `/codex-collab:flow`。
...
```
/codex-collab:flow 优化 ./src/sort.go 的排序性能
```
...
普通使用走 `/codex-collab:flow` 即可；如需直接调用 `codex` 工具（高级），展开查看参数与返回：
